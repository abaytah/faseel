'use client';

import { motion } from 'framer-motion';
import Link from 'next/link';
import { ArrowRight, FileText, CalendarClock, Shield, AlertCircle } from 'lucide-react';

const containerVariants = {
  hidden: { opacity: 0 },
  visible: { opacity: 1, transition: { staggerChildren: 0.08 } },
};

const itemVariants = {
  hidden: { opacity: 0, y: 16 },
  visible: { opacity: 1, y: 0, transition: { type: 'spring', stiffness: 260, damping: 20 } },
};

export default function ContractPage() {
  return (
    <motion.div
      variants={containerVariants}
      initial="hidden"
      animate="visible"
      className="space-y-4"
    >
      {/* Back */}
      <motion.div variants={itemVariants}>
        <Link
          href="/tenant/dashboard"
          className="inline-flex items-center gap-1 text-sm text-[var(--muted-foreground)] transition-colors hover:text-[var(--foreground)]"
        >
          <ArrowRight className="h-4 w-4" />
          العودة
        </Link>
      </motion.div>

      {/* Coming Soon */}
      <motion.div
        variants={itemVariants}
        className="shadow-soft rounded-2xl border border-[var(--border)] bg-[var(--card)] p-8 text-center"
      >
        <div className="mb-4 flex justify-center">
          <div className="bg-brand-50 dark:bg-brand-900/20 flex h-16 w-16 items-center justify-center rounded-2xl">
            <FileText className="text-brand-500 h-8 w-8" />
          </div>
        </div>
        <h3 className="mb-2 text-base font-bold">عقد الإيجار</h3>
        <p className="mb-4 text-sm text-[var(--muted-foreground)]">
          قريبا ستتمكن من عرض تفاصيل عقد الإيجار الخاص بك، بما في ذلك رقم إيجار المرجعي، تفاصيل
          الأطراف، والمعلومات المالية.
        </p>
        <div className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-4 py-2 text-xs font-medium text-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
          <AlertCircle className="h-3.5 w-3.5" />
          قريبا
        </div>
      </motion.div>

      {/* Expected Features */}
      <motion.div
        variants={itemVariants}
        className="shadow-soft rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5"
      >
        <h3 className="mb-4 text-sm font-bold">المزايا القادمة</h3>
        <div className="space-y-3">
          {[
            {
              icon: Shield,
              label: 'عرض عقد إيجار موثّق في منصة إيجار',
              color: 'text-emerald-500',
              bg: 'bg-emerald-50 dark:bg-emerald-900/20',
            },
            {
              icon: CalendarClock,
              label: 'تاريخ البداية والنهاية مع العد التنازلي',
              color: 'text-amber-500',
              bg: 'bg-amber-50 dark:bg-amber-900/20',
            },
            {
              icon: FileText,
              label: 'التفاصيل المالية وشروط العقد',
              color: 'text-sky-500',
              bg: 'bg-sky-50 dark:bg-sky-900/20',
            },
          ].map((feature) => (
            <div
              key={feature.label}
              className="flex items-center gap-3 rounded-xl bg-[var(--secondary)] p-3"
            >
              <div
                className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${feature.bg}`}
              >
                <feature.icon className={`h-4 w-4 ${feature.color}`} />
              </div>
              <p className="text-xs font-medium">{feature.label}</p>
            </div>
          ))}
        </div>
      </motion.div>
    </motion.div>
  );
}
